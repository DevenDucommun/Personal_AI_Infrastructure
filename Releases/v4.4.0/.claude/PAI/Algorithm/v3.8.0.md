...
**Voice (FIRST action after loading this file, if voice enabled):** Announce `"Entering the Algorithm"` via the voice utility. Skip if `voice.enabled` is false.
...
For Extended+, immediately after voice curl:
